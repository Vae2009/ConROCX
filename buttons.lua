...
function ConROC:UpdateLockTexture()
    ...
    if ConROC.db.profile.unlockWindow then
        lockButton.lockTexture:SetTexture("Interface\\AddOns\\ConROCX\\images\\padlock_open")
    else
        lockButton.lockTexture:SetTexture("Interface\\AddOns\\ConROCX\\images\\padlock_closed")
    end
...
function ConROC:SlashUnlock()
	...
	if IsAddOnLoaded("ConROCX_Rogue") or IsAddOnLoaded("ConROCX_Shaman") then
		if ConROC.db.profile.unlockWindow then
...
function ConROC:DisplayToggleFrame()
	...
	local mframe = CreateFrame("Frame", "ConROCToggleMover", UIParent)
		...
		local t = mframe.texture;
			if not t then
				...
				t:SetTexture('Interface\\AddOns\\ConROCX\\images\\magiccircle-purge');
	...
	local ontex = otbutton:CreateTexture()
		ontex:SetTexture("Interface\\AddOns\\ConROCX\\images\\buttonUp")
	...
	local ohtex = otbutton:CreateTexture()
		ohtex:SetTexture("Interface\\AddOns\\ConROCX\\images\\buttonHighlight")
		ohtex:SetTexCoord(0, 0.625, 0, 0.6875)
		ohtex:SetAllPoints()
		otbutton:SetHighlightTexture(ohtex)

	local optex = otbutton:CreateTexture()
		optex:SetTexture("Interface\\AddOns\\ConROCX\\images\\buttonDown")
	...
	local ntex = tbutton:CreateTexture()
		ntex:SetTexture("Interface\\AddOns\\ConROCX\\images\\buttonUp")
	...
	local htex = tbutton:CreateTexture()
		htex:SetTexture("Interface\\AddOns\\ConROCX\\images\\buttonHighlight")
		htex:SetTexCoord(0, 0.625, 0, 0.6875)
		htex:SetAllPoints()
		tbutton:SetHighlightTexture(htex)

	local ptex = tbutton:CreateTexture()
		ptex:SetTexture("Interface\\AddOns\\ConROCX\\images\\buttonDown")
...
function ConROC:DisplayWindowFrame()
	...
	local t = frame.texture;
		if not t then
			...
			t:SetTexture('Interface\\AddOns\\ConROCX\\images\\Bigskull');
			t:SetBlendMode('BLEND');
...
function ConROC:DefenseWindowFrame()
	...
	local t = frame.texture;
		if not t then
			t = frame:CreateTexture(nil, "ARTWORK")
			t:SetTexture('Interface\\AddOns\\ConROCX\\images\\shield2');
			t:SetBlendMode('ADD');
			t:SetAlpha(ConROC.db.profile.transparencyWindow);
...
function ConROC:InterruptWindowFrame()
	...
	local t = frame.texture;
		if not t then
			t = frame:CreateTexture(nil, "ARTWORK")
			t:SetTexture('Interface\\AddOns\\ConROCX\\images\\lightning-interrupt');
			t:SetBlendMode('BLEND');
			t:SetAlpha(ConROC.db.profile.transparencyWindow);
			t:SetVertexColor(.1, .1, .1);
			frame.texture = t;
...
function ConROC:SpellmenuFrame()
	...
	local frameTitle = CreateFrame("Frame", "ConROCSpellmenuFrame_Title", frame)
		...
		frameTitle:SetScript("OnMouseDown", function (self, otbutton, up)
			if ConROC.db.profile.unlockWindow then
					frame:StartMoving()
			end
		end)

		frameTitle:SetScript("OnMouseUp", function (self, otbutton, up)
			if ConROC.db.profile.unlockWindow then
...
function ConROC:CreateDamageOverlay(parent, id)
	...
	if not t then
		...
		t:SetTexture('Interface\\AddOns\\ConROCX\\images\\skull');
		t:SetBlendMode('ADD');
		frame.texture = t;
	end
...
function ConROC:CreateInterruptOverlay(parent, id)
	...
	if not t then
		...
		t:SetTexture('Interface\\AddOns\\ConROCX\\images\\lightning');
		t:SetBlendMode('BLEND');
		frame.texture = t;
	end
...
function ConROC:CreatePurgableOverlay(parent, id)
	...
	if not t then
		...
		t:SetTexture('Interface\\AddOns\\ConROCX\\images\\magiccircle');
		t:SetBlendMode('BLEND');
		frame.texture = t;
	end
...
function ConROC:CreateTauntOverlay(parent, id)
	...
	if not t then
		...
		t:SetTexture('Interface\\AddOns\\ConROCX\\images\\rage');
		t:SetBlendMode('BLEND');
		frame.texture = t;
	end
...
function ConROC:CreateRaidBuffsOverlay(parent, id)
	...
	if not t then
		...
		t:SetTexture('Interface\\AddOns\\ConROCX\\images\\plus');
		t:SetBlendMode('BLEND');
		frame.texture = t;
	end
...
function ConROC:CreateMovementOverlay(parent, id)
	...
	if not t then
		...
		t:SetTexture('Interface\\AddOns\\ConROCX\\images\\arrow');
		t:SetBlendMode('BLEND');
		frame.texture = t;
	end
...
function ConROC:CreateCoolDownOverlay(parent, id)
	...
	if not t then
		...
		t:SetTexture('Interface\\AddOns\\ConROCX\\images\\starburst');
		t:SetBlendMode('ADD');
		frame.texture = t;
	end
